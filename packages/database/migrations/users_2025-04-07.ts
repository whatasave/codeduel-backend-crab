...
export async function up(db: Kysely<unknown>): Promise<void> {
  ...
  await sql`
    DROP TRIGGER IF EXISTS update_user_timestamp ON ${sql.table(TABLE_NAME)};
...
export async function down(db: Kysely<unknown>): Promise<void> {
  ...
  await sql`DROP TRIGGER IF EXISTS update_user_timestamp ON ${sql.table(TABLE_NAME)};`.execute(db);
